...
```bash
git clone https://github.com/<votre-compte>/analyse-financiere-entreprise.git ~/.claude/skills/analyse-financiere-entreprise
```
...
Un hook `pre-commit` régénère `analyse-financiere-entreprise.zip` automatiquement dès que `SKILL.md`, `references/formules.md` ou un fichier de `examples/` fait partie du commit.
...
zip -r analyse-financiere-entreprise.zip SKILL.md references/ examples/
```
